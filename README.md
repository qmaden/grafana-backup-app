...
2. Have versioned backups `(date and time in file name)` for restoring and saving to Azure Storage with Workload Identity support.

## Supported components
* **Folders** - Grafana folder structure
* **Folder Permissions** - Access control for folders
* **Library Elements** - Reusable dashboard panels (doesn't work with Grafana 8.0.0 but 8.4.3)
* **Dashboards** - All dashboard configurations (contains embedded alerts)
* **Datasources** - Data source configurations
* **Alert Channels** - Notification channels for alerts
* **Alert Rules** - Alert rule definitions (Supported in version 9.4.0 of Grafana and up)
* **Snapshots** - Dashboard snapshots
* **Dashboard Versions** - Dashboard version history (backup only, no restore)
* **Annotations** - Dashboard annotations
* **Contact Points** - Modern alerting contact points
* **Notification Policies** - Modern alerting notification policies

**NOTE** The only supported `orgId` right now is `1`, the default organization will be backed up only!

## Requirements
* Bash
* Python 3.11 or later (recommended: Python 3.13)
...
### Example Config
* Copy [grafanaSettings.example.json](examples/grafanaSettings.example.json) and modify it for your Azure environment.
...
**Note:** This version requires Python 3.11 or later. For best performance and latest features, Python 3.13 is recommended.
...
```
Installation works with `pip` or `uv` (recommended for faster installs):
```bash
# Using pip
pip install .
...
***Azure Example:*** Set Azure configurations in `-e` or `grafanaSettings.json`([example](examples/grafanaSettings.example.json))
```
		   -e AZURE_STORAGE_CONTAINER_NAME="azure-storage-container-name" \
		   -e AZURE_STORAGE_ACCOUNT_NAME="yourstorageaccount" \
		   -e AZURE_MANAGED_IDENTITY_CLIENT_ID="your-managed-identity-client-id"
```

For secure Azure Workload Identity authentication, see [README-Azure.md](README-Azure.md) for complete setup instructions.
...
This version is optimized for Azure environments and includes:
- **Azure Workload Identity** support for secure, keyless authentication
- **Managed Identity** integration for Azure resources
- **Azure Storage** backend with retry logic and error handling

See [README-Azure.md](README-Azure.md) for detailed Azure deployment instructions including Kubernetes configurations.
